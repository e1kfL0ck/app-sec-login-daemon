...
def send_password_reset_email(to_email: str, reset_link: str) -> bool:
    """
    Send a password reset email using SMTP. Returns True on success, False on failure.
    ...
    if not mail_username or not mail_password:
        logger.warning(
            "Mail credentials not configured; skipping sending password reset email to %s",
            to_email,
        )
        return False

    msg = EmailMessage()
    msg["Subject"] = "Reset your password - AppSec"
    msg["From"] = mail_from
    msg["To"] = to_email
