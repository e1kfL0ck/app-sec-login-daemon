...
\texttt{src/app.py} registers all blueprints. \texttt{src/auth/routes.py}, \texttt{src/content/routes.py}, and
\texttt{src/user/routes.py} define the route groups. \texttt{src/auth/services.py}, \texttt{src/content/services.py}, and
\texttt{src/user/services.py} provide business logic; repositories live in the corresponding \texttt{repository.py}
files. Validation helpers are in \texttt{src/auth/validators.py}, \texttt{src/content/validators.py}, and
\texttt{src/user/validators.py}. Session enforcement is implemented in \texttt{src/session\_helpers.py}.\subsection*{Architecture map}
\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Layer} & \textbf{Primary modules} \\
\hline
Routes & \texttt{src/auth/routes.py}, \texttt{src/content/routes.py}, \texttt{src/user/routes.py} \\
\hline
Services & \texttt{src/auth/services.py}, \texttt{src/content/services.py}, \texttt{src/user/services.py} \\
\hline
Repositories & \texttt{src/auth/repository.py}, \texttt{src/content/repository.py}, \texttt{src/user/repository.py} \\
\hline
Validation & \texttt{src/auth/validators.py}, \texttt{src/content/validators.py}, \texttt{src/user/validators.py} \\
\hline
Session/Auth & \texttt{src/session\_helpers.py} \\
\hline
\end{tabular}
\end{center}
...
\paragraph{Implementation references}
Auth routes are in \texttt{src/auth/routes.py}. Core login and token workflows are in \texttt{src/auth/services.py}.
Token helpers are in \texttt{src/auth/tokens.py}. MFA logic is handled by \texttt{src/auth/mfa.py} with templates under
\texttt{src/auth/templates/}. User disablement and login checks are enforced via \texttt{src/user/services.py} and
repository calls in \texttt{src/user/repository.py}.
...
\paragraph{Implementation references}
Content routes are in \texttt{src/content/routes.py}. Business logic and pagination live in
\texttt{src/content/services.py}. Data access is in \texttt{src/content/repository.py}, and permission checks are defined
in \texttt{src/content/permissions.py}. Templates reside in \texttt{src/content/templates/}.
...
accounts.
\paragraph{Implementation references}
User management routes are in \texttt{src/user/routes.py}. Account operations are implemented in
\texttt{src/user/services.py} with persistence in \texttt{src/user/repository.py}. Templates are located in
\texttt{src/user/templates/}.
...
\paragraph{Implementation references}
Shared UI layout uses \texttt{src/templates/base.html} and \texttt{src/templates/dashboard.html}. Content-specific
templates are under \texttt{src/content/templates/}, user templates under \texttt{src/user/templates/}, and auth
templates under \texttt{src/auth/templates/}. Static assets are in \texttt{src/static/} and
\texttt{src/content/static/}.

\subsection*{Template inventory}
\begin{center}
\begin{tabular}{|l|l|}
\hline
\textbf{Area} & \textbf{Templates added} \\
\hline
Auth & \texttt{activation.html} \\
\hline
Content & \texttt{feed.html}, \texttt{admin\_feed.html}, \texttt{post\_create.html}, \texttt{post\_edit.html}, \texttt{post\_detail.html}, \texttt{search.html}, \texttt{user\_posts.html} \\
\hline
User & \texttt{profile.html}, \texttt{settings.html}, \texttt{admin\_users.html} \\
\hline
\end{tabular}
\end{center}
...
introduced by the merge.
\paragraph{Implementation references}
Schema creation and seed data are defined in \texttt{src/init\_db.py}. Environment samples are in \texttt{.env.example}.
Container startup uses \texttt{src/entrypoint.sh}. Build and orchestration are handled by \texttt{Dockerfile} and
...
\hline
users & Accounts, roles, activation, MFA, disabled state \\
\hline
tokens & Activation and password reset tokens \\
\hline
posts & Blog posts with public/private visibility \\
\hline
