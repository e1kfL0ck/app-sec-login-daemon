...
\begin{itemize}
  ...
  \item Example tables and columns:
        \begin{itemize}
          \item \texttt{users}: \texttt{id}, \texttt{email}, \texttt{password\_hash}, \texttt{last\_login}, \texttt{nb\_failed\_logins}, \texttt{created\_at}, \texttt{activated} (0/1)
          \item \texttt{tokens}: \texttt{token}, \texttt{user\_id}, \texttt{expires\_at}, \texttt{created\_at}, \texttt{type} (``activation'' / ``password\_reset'')
        \end{itemize}
...
\sectionTitle{Login and Session Management Module}
...
\begin{itemize}
  ...
  \item Password -- chosen by the user to authenticate and establish a session.
\end{itemize}
...
\begin{description}
  ...
  \item [FR-10] The system must send a password reset link containing the token to the user's email.
...
\begin{description}
  \item [NFR-U01] Error and validation messages from the frontend must be clear and user-friendly.
  \item [NFR-U02] Login should require no more than one user interaction step.
\end{description}

\paragraph{Performance Requirements}
\begin{description}
  \item [NFR-P01]: Login requests should be processed with low latency.
  \item [NFR-P02]: The system should scale to the required number of concurrent logins.
\end{description}
