...
\begin{description}
  \item [NFR-S01] All input must be treated as untrusted and validated on the server.
  \item [NFR-S02] Passwords must be stored using secure, modern cryptographic hashingitemize.
  \item [NFR-S03] The system must be protected from SQL injection, XSS, CSRF, command injections.
  \item [NFR-S04] Sensitive information (passwords, tokens) must not be logged.
  \item [NFR-S05] All communication between client and server should use HTTPS.
\end{description}

\paragraph{Usability Requirements}
\begin{description}
  \item [NFR-U01] Error and validation messages must be clear and user-friendly.
...
\begin{itemize}
  ...
  \item Flask: Web framework for routing (for example, \texttt{POST /register} and \texttt{GET /activate/<token>}), input validation, and returning HTML or JSON responses.
\end{itemize}
...
\vspace{1em}
\noindent
\textbf{Notes:} This document formats the requirements and design notes for the user registration component. If you want, I can:
\begin{itemize}
  \item generate a minimal \texttt{README} with build and run instructions,
  \item produce example SQL \texttt{CREATE TABLE} statements,
  \item or compile this document to PDF on your machine and show issues.
\end{itemize}
