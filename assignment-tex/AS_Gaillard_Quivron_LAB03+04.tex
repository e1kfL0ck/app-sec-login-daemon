...
This report documents the implemented Flask + SQLite prototype, covering requirements, flows, and supporting assets for LAB-03 (registration) and LAB-04 (login/session).

\part{Lab-03: User Registration Module}
...
\begin{description}
  \item [NFR-S01] Passwords must be stored using secure, modern cryptographic hashing algorithms.
...
\part{Lab-04: Login and Session Management Module}
\chapter{Component Overview}
\section{Component Short Description}
...
Once a user enables Multi-Factor Authentication (MFA) from their account settings, the backend generates a unique Time-based One-Time Password (TOTP) secret using the \texttt{pyotp} library. This secret is then converted into a QR code using the \texttt{qrcode} library and displayed to the user, allowing them to enroll their account in an authenticator application such as Google Authenticator or Authy.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{img/front_mfa_setup.png}
  \caption{MFA setup page displaying the QR code for enrollment in an authenticator application.}
  \label{fig:mfa_setup}
\end{figure}

During the login process, after the user successfully submits their email and password, the backend checks whether MFA is enabled for the account. If so, the user is redirected to a dedicated MFA verification page and prompted to enter the current TOTP code generated by their authenticator application.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{img/front_mfa.png}
  \caption{MFA verification page prompting the user to enter a TOTP code.}
  \label{fig:mfa_verification}
\end{figure}

The submitted code is sent to the backend, which verifies it against the expected value generated from the stored TOTP secret. If the code is valid, the authentication process is completed and access is granted. If the verification fails, an error message is returned and displayed to the user.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{img/front_mfa_error.png}
  \caption{Error message shown when an invalid MFA code is submitted.}
  \label{fig:mfa_error}
\end{figure}

To ensure account recovery, the system also generates a set of single-use backup codes during the MFA setup process. These codes can be used as an alternative authentication method if the user loses access to their authenticator application. Overall, this process strengthens account security by adding a second authentication factor beyond the password.

\subsection{Handling unwanted interactions}
\begin{itemize}
  \item Tokens are expired after a short duration if unused.
  \item Tokens are invalidated immediately after use to prevent replay attacks.
  \item Session cookies are invalidated on logout and rotated after authentication.
  \item Password-reset and activation tokens are single-use and expire quickly to reduce replay risk.
  \item Generic error messages are provided for failed login attempts.
\end{itemize}
...
\subsection{MFA Setup Process}
\includegraphics[width=\textwidth]{img/UML_SetupMFA.pdf}

\subsection{MFA Verification Process}
\includegraphics[width=\textwidth]{img/UML_Login.pdf}

\section{Front-End Screens}
\subsection{Welcome Screen}
\includegraphics[width=\textwidth]{img/front_welcome.png}

\subsection{Dashboard}
\includegraphics[width=\textwidth]{img/front_dashboard.png}

\subsection{Registration}
\includegraphics[width=\textwidth]{img/front_register.png}

\subsection{Login With Error}
\includegraphics[width=\textwidth]{img/front_login_with_error.png}

\subsection{Password Reset Request}
\includegraphics[width=\textwidth]{img/front_forgotten_password.png}
